...
import { arrayStartsWith } from "./util/arrayStartsWith";
...
const validateRojo = new Lazy(() =>
	ajv.compile(JSON.parse(fs.readFileSync(SCHEMA_PATH).toString())),
);
...
export class Project {
	...
	private parseConfig(rojoConfigFilePath: string, doNotPush = false) {
		...
		if (fs.pathExistsSync(realPath)) {
			...
			if (isValidRojoConfig(configJson)) {
				...
				warn(
					`RojoResolver: Invalid configuration! ${ajv.errorsText(
						validateRojo.get().errors,
					)}`,
				);
			}
		} else {
	...
	private parsePath(itemPath: string) {
		const realPath = fs.pathExistsSync(itemPath)
			? fs.realpathSync(itemPath)
			: itemPath;
		if (path.extname(itemPath) === LUA_EXT) {
			...
			if (
				isDirectory &&
				fs.readdirSync(realPath).includes(ROJO_DEFAULT_NAME)
			) {
	...
	public getFileRelation(
		fileRbxPath: RbxPath,
		moduleRbxPath: RbxPath,
	): FileRelation {
		const fileContainer = this.getContainer(
			this.isolatedContainers,
			fileRbxPath,
		);
		const moduleContainer = this.getContainer(
			this.isolatedContainers,
			moduleRbxPath,
		);
	...
	public isIsolated(rbxPath: RbxPath) {
		return (
			this.getContainer(this.isolatedContainers, rbxPath) !== undefined
		);
	}
